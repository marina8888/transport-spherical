...
from src.plotter.numerical_only import plotter_domain, plotter_single
...
def main():
    run_flame_domain("okafor-2017.cti", '/stagnation/CH4_NH3/20%_data_jp_symp.csv', flame_type='stagnation')
    # run_flame("okafor-2017.cti", "freely_prop/CH4_NH3/icfd_lbv_test.csv", flame_type="freely_prop")

    # plotting example:
    # plotter_single("1000grid/freely_prop_CH4_NH3", 'x_col', 'T_max', 'adiabatic flame temperature, '+ r"$\mathrm{T_{A}}$" + ' (K)', 'ammonia heat ratio, '+ r"$\mathrm{E_{NH3}}$", ['0.1MPa', '0.5MPa'])


    # plotting domain example, mech only:
    # plotter_domain("numerical_domain/stagnation_CH4_NH3/20%_rich.csv")

    # error calculation:
    # error_object = ErrorCalculator("1000grid/stagnation_H2_NH3/10%", 'stagnation/NH3_H2/10%_data_full.csv', "stagnation")

    #sensitivity calculation:
    # run_rop_sens("gotama.cti", "stagnation/NH3_H2/20%_data_full.csv", flame_type="stagnation", species='N2O')
